...
def test_runtime_owns_learning_service():
 src=(Path(__file__).parents[2]/'custom_components'/'plant_helper'/'runtime.py').read_text();assert 'learning: Any = None' in src
def test_learning_uses_separate_store_sections():
 src=(Path(__file__).parents[2]/'custom_components'/'plant_helper'/'domain'/'learning.py').read_text()
