...
def test_state_tracking_and_listener_lifecycle():
 root=Path(__file__).parents[2]/'custom_components'/'plant_helper';src=(root/'physical.py').read_text()
...
def test_fixed_debounce_and_cached_environment_contract():
 src=(Path(__file__).parents[2]/'custom_components'/'plant_helper'/'domain'/'physical.py').read_text()
