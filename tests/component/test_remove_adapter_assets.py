...
def test_remove_flow_and_registry_hooks():
 root=Path(__file__).parents[2]/'custom_components'/'plant_helper';src=(root/'options.py').read_text()
...
def test_translations_and_runtime_hooks():
 root=Path(__file__).parents[2]/'custom_components'/'plant_helper';a=json.loads((root/'strings.json').read_text());b=json.loads((root/'translations/en.json').read_text());assert a==b
