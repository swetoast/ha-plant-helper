...
def test_runtime_adapter_uses_runtime_data_and_platform_forwarding():
 ...
 assert "entry.runtime_data.plants.unload()" in init
...
def test_dynamic_platform_contract():
 ...
 for name in ("sensor.py","binary_sensor.py"):
  ...
  assert "async_add_entities(entities)" in source
...
def test_entity_identity_contract():
 source=(Path(__file__).parents[1]/"custom_components"/"plant_helper"/"entity.py").read_text()
