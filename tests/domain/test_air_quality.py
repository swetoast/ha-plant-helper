...
def test_current_hourly_override_refresh_generation():
 c=AirQualityCollector(lambda _:asyncio.sleep(0,result=data()));r=request_for(1,2,True);s=run(c.refresh(r,NOW));assert s.source=='open_meteo' and s.current_ozone==55 and len(s.hourly_ozone)==3 and s.generation==1
