...
class INaturalistProvider:
    ...
    async def resolve(self, query: str) -> ProviderResult:
        ...
        try:
            ...
            if payload is None:
                return ProviderResult(False, "inaturalist", api_checked=True, api_called=bool(_calls()), calls_made=_calls(), message=self.last_error or "iNaturalist autocomplete failed")
            ...
            if taxon is None and results:
                taxon = results[0]
            if taxon is None:
                return ProviderResult(False, "inaturalist", api_checked=True, api_called=bool(_calls()), calls_made=_calls(), message="No iNaturalist match")
