...
class PlantHelperOptionsFlow(OptionsFlow):
    ...
    async def async_step_edit_plant(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            if not errors:
                name, species, entities = split_record(user_input)
                merged = {**(record.get("entities") or {}), **entities}
                await storage.async_update_user_plant(
                    self._edit_id, {"custom_name": name, "entities": merged}
                )
                return self._finish()
...
_REMOVE_INFO = "Removing a plant deletes its entities. Its learned baseline is kept unless you also reset the database."
_SETTINGS_INFO = (
